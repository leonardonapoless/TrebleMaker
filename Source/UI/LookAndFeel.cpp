...
{
    const auto background    = juce::Colour(0xFFF2F2F2);
    const auto gridLines     = juce::Colour(0xFFD0D0D0);
    const auto textDark      = juce::Colour(0xFF222222); 
    const auto textLight     = juce::Colour(0xFF666666); 
    
    const auto screenBezelStart = juce::Colour(0xFFCCCCCC);
    const auto screenBezelEnd   = juce::Colour(0xFFF0F0F0);
    
    const auto screenBackground = juce::Colour(0xFFE8E0E0);
    const auto screenRed        = juce::Colour(0xFFFF3B30);
    const auto knobTick         = juce::Colour(0xFFFF3B30); 
    const auto buttonBlue       = juce::Colour(0xFF101010);

    const auto knobBodyLight    = juce::Colour(0xFFEEEEEE);
    const auto knobBodyDark     = juce::Colour(0xFF999999);
    const auto knobBodyMid      = juce::Colour(0xFFCCCCCC);
    const auto knobFaceNormal   = juce::Colour(0xFFF5F5F5);
                      ...
                      const float rotaryEndAngle, juce::Slider&)
{
    ...
    juce::ColourGradient conicalGrad(theme_colors::knobBodyLight, center.x, center.y,
                                     theme_colors::knobBodyDark, center.x + radius, center.y + radius, true);
    ...
    juce::ColourGradient topGrad(theme_colors::knobFaceNormal, center.x - faceRadius, center.y - faceRadius,
                                 theme_colors::knobFaceShadow, center.x + faceRadius, center.y + faceRadius, false);
                          ...
                          bool shouldDrawButtonAsDown)
{
    ...
    juce::ColourGradient bezelGrad(juce::Colour(0xFF444444), 0, bounds.getY(),
                                   juce::Colour(0xFF222222), 0, bounds.getBottom(), false);
